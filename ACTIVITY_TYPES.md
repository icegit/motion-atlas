...
Standard Garmin activities use the authorized Garmin activity glyph set. Non-Garmin icons are reserved for Garmin `Other`/`Custom` records whose private activity name reliably identifies an activity that Garmin does not provide as a specific type. These name-aware fallbacks are not limited to `submarine` and `hot_air_balloon`; the complete supported set is documented below.
...
Garmin extraction provenance is recorded in `docs/activity-icons/GARMIN-SOURCE.md`. Icons used only for the documented name-derived `Other`/`Custom` fallbacks come from Tabler Icons 3.46.0 and retain the included MIT license in `docs/activity-icons/LICENSE-TABLER.txt`.
